...
**落地结果（2026-09-06，形态改为"lib.rs 上下文 LRU 复用"而非 PyO3 class——复用逻辑留在 Rust 侧，未来方案 R 可直接复用）**：先按方案 E 同款纪律插桩实测，**推翻了本节动机的 5~20 ms 估计——av1_nvenc 上下文打开实测 ~80 ms/次**（RTX 4070），占 512×512 单图墙钟 ~75%；auto_cq 每图 3 次（2 次 512×512 试编码 + 1 次全分辨率，~240 ms）。实现：`src/lib.rs` 进程内 LRU 缓存，key = (宽，高，像素格式，preset，cq) 全部打开选项（constqp 烧在会话里）；命中路径不 send EOF（delay=0 + intra-only，且 FFmpeg nvenc 对 AV1 硬编码 `repeatSeqHdr=1`，每包自包含）；容量经 `NVAVIF_CTX_CACHE`（默认 1 = 峰值会话数与旧行为相同；compress_dir 设 `max(1, 8//workers)` 守住驱动 8 会话预算）。正确性由 `ctx_reuse_tests` 锁定：复用会话输出与全新会话解码 MAE=0.0、独立可解码。**实测收益**：同尺寸小图 107.9 → 4.3 ms/张（**25×**，批量端到端 200 张同尺寸语料 19 → 45 MP/s）；容量 ≥3 时 auto_cq 试编码上下文与源尺寸无关——任意语料 335 → 108 ms（3.1×），同尺寸 354 → 22.5 ms（15.7×）；尺寸互异 + 固定 cq 语料零收益零伤害（全量 79 张输出与改前**逐字节一致**，墙钟 8.54 → 8.44 s）。**两条教训**：① miss 路径必须**先驱逐再开**——"先开后驱逐"让每 worker 瞬时持 2 个会话，8 worker 超 8 会话上限且显存翻倍（峰值 9.1 GB），语料最大图 NVENC 初始化失败静默回落 CPU（1.65 s → 70 s，输出变小易被误读为压缩率改善）；② 常驻上下文是拿显存换时间（本机 12 GB 峰值 8.1 GB 无压力，小显存需下调 worker/容量）。详见 DEVELOPMENT_NOTES §16。
...
| ~~**P2**~~ ✅ | 方案 Q：NVENC 上下文 LRU 复用 | **已落地（2026-09-06）**：实测 ~80 ms/开（推翻 5~20 ms 估计）；同尺寸小图 25×，auto_cq（容量≥3）3.1~15.7×；全量语料输出逐字节一致 | 中（须守住 8 会话预算：先驱逐再开） | ~120 行 Rust + 测试 |
...
8. **生产化路线（2026-09-06 评估）**：当前"Python 编排 + Rust 编码核心"适合批量生产（Python 非瓶颈，NVENC 已可打满）；~~先做方案 Q~~ ✅ 方案 Q 已落地（2026-09-06，上下文 LRU 复用，实测收益远超预期，见落地结果与 DEVELOPMENT_NOTES §16）；确有单二进制交付/无人值守需求时再做方案 R（只重写编排层，报表 schema 与 Python 版一致以便 `compare_runs.py` 回归；验收清单已固化为 `PARITY_MATRIX.md`）。
