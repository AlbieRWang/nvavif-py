use std::sync::atomic::{AtomicI8, AtomicBool, Ordering};
use pyo3::prelude::*;
...
struct MatrixCoefs {
    yr: f32, yg: f32, yb: f32,
    ur: f32, ug: f32, ub: f32,
    vr: f32, vg: f32, vb: f32,
}
    ...
    fn new(matrix: PyColorMatrix) -> Self {
        match matrix {
            PyColorMatrix::Bt601 => Self {
                ...
                ur: -0.168736,  ug: -0.331264,  ub: 0.5,
                vr: 0.5,        vg: -0.418688,  vb: -0.081312,
            },
            PyColorMatrix::Bt709 => Self {
                yr: 0.2126,     yg: 0.7152,     yb: 0.0722,
                ur: -0.114572,  ug: -0.385428,  ub: 0.5,
                vr: 0.5,        vg: -0.454153,  vb: -0.045847,
            },
            PyColorMatrix::Bt2020 => Self {
                yr: 0.2627,     yg: 0.6780,     yb: 0.0593,
                ur: -0.13963,   ug: -0.36037,   ub: 0.5,
                vr: 0.5,        vg: -0.45979,   vb: -0.04021,
            },
        }
    }
}
...
struct ReaderU8<'a> { data: &'a [u8], channels: usize }
impl PixelReader for ReaderU8<'_> {
    ...
    fn read(&self, idx: usize) -> (f32, f32, f32, f32) {
        ...
        let a = if self.channels == 4 { self.data[idx+3] as f32 / 255.0 } else { 1.0 };
        (r, g, b, a)
    }
}

struct ReaderU16<'a> { data: &'a [u16], channels: usize }
impl PixelReader for ReaderU16<'_> {
    ...
    fn read(&self, idx: usize) -> (f32, f32, f32, f32) {
        ...
        let a = if self.channels == 4 { self.data[idx+3] as f32 / 65535.0 } else { 1.0 };
        (r, g, b, a)
    }
}

struct ReaderF32<'a> { data: &'a [f32], channels: usize }
impl PixelReader for ReaderF32<'_> {
    ...
    fn read(&self, idx: usize) -> (f32, f32, f32, f32) {
        ...
        let a = if self.channels == 4 { self.data[idx+3].clamp(0.0, 1.0) } else { 1.0 };
        (r, g, b, a)
    }
}
...
fn is_hardware_supported() -> bool {
    ...
    if let Some(codec) = ffmpeg_next::encoder::find_by_name("av1_nvenc") {
        let encoder_ctx = ffmpeg_next::codec::context::Context::new_with_codec(codec);
        if let Ok(mut video_ctx) = encoder_ctx.encoder().video() {
            video_ctx.set_width(256);
            video_ctx.set_height(256);
            ...
            options.set("tune", "hq");
            supported = video_ctx.open_as_with(codec, options).is_ok();
        }
    }
...
fn extract_yuv420<R: PixelReader>(reader: &R, width: usize, height: usize, channels: usize, depth: PyColorDepth, matrix: PyColorMatrix, is_alpha: bool) -> YuvData {
    ...
    match depth {
        PyColorDepth::EightBit => {
            ...
            y_plane.par_chunks_mut(width).enumerate().for_each(|(y, row)| {
                for x in 0..width {
                    ...
                    if is_alpha {
                        ...
                        row[x] = ((coefs.yr * r + coefs.yg * g + coefs.yb * b) * 255.0).clamp(0.0, 255.0) as u8;
                    }
                }
            });

            if !is_alpha {
                uv_plane.par_chunks_mut(width).enumerate().for_each(|(y_sub, row_uv)| {
                    for x_sub in 0..(width / 2) {
                        let idx = (y_sub * 2 * width + x_sub * 2) * channels;
                        let (r, g, b, _a) = reader.read(idx);
                        row_uv[x_sub * 2] = ((coefs.ur * r + coefs.ug * g + coefs.ub * b) * 255.0 + 128.0).clamp(0.0, 255.0) as u8;
                        row_uv[x_sub * 2 + 1] = ((coefs.vr * r + coefs.vg * g + coefs.vb * b) * 255.0 + 128.0).clamp(0.0, 255.0) as u8;
                    }
                });
            }
            YuvData { y: y_plane, u: uv_plane, v: vec![], pixel_format: Pixel::NV12 }
        }
        PyColorDepth::TenBit => {
            ...
            y_plane.par_chunks_mut(width).enumerate().for_each(|(y, row)| {
                for x in 0..width {
                    ...
                    if is_alpha {
                        ...
                        row[x] = (((coefs.yr * r + coefs.yg * g + coefs.yb * b) * 1023.0).clamp(0.0, 1023.0) as u16) << 6;
                    }
                }
            });
            ...
            if !is_alpha {
                uv_plane.par_chunks_mut(width).enumerate().for_each(|(y_sub, row_uv)| {
                    for x_sub in 0..(width / 2) {
                        let idx = (y_sub * 2 * width + x_sub * 2) * channels;
                        let (r, g, b, _a) = reader.read(idx);
                        let u_val = (coefs.ur * r + coefs.ug * g + coefs.ub * b) * 1023.0 + 512.0;
                        let v_val = (coefs.vr * r + coefs.vg * g + coefs.vb * b) * 1023.0 + 512.0;
            ...
            YuvData {
                ...
                v: vec![], pixel_format: Pixel::P010LE
            }
        }
    }
}
...
/// Parallelized row-wise processing.
fn extract_yuv444<R: PixelReader>(reader: &R, width: usize, height: usize, channels: usize, depth: PyColorDepth, matrix: PyColorMatrix, is_alpha: bool) -> YuvData {
    let num_pixels = width * height;
    let coefs = MatrixCoefs::new(matrix);
    ...
    match depth {
        PyColorDepth::EightBit => {
            ...
            y.par_chunks_mut(width).zip(u.par_chunks_mut(width)).zip(v.par_chunks_mut(width))
                .enumerate().for_each(|(y_idx, ((row_y, row_u), row_v))| {
                    for x in 0..width {
                        ...
                        if is_alpha {
                            ...
                            row_y[x] = ((coefs.yr * r + coefs.yg * g + coefs.yb * b) * 255.0).clamp(0.0, 255.0) as u8;
                            row_u[x] = ((coefs.ur * r + coefs.ug * g + coefs.ub * b) * 255.0 + 128.0).clamp(0.0, 255.0) as u8;
                            row_v[x] = ((coefs.vr * r + coefs.vg * g + coefs.vb * b) * 255.0 + 128.0).clamp(0.0, 255.0) as u8;
                        }
                    }
                });
            YuvData { y, u, v, pixel_format: Pixel::YUV444P }
        }
        PyColorDepth::TenBit => {
                ...
                .enumerate().for_each(|(y_idx, ((row_y, row_u), row_v))| {
                    for x in 0..width {
                        ...
                        if is_alpha {
                            ...
                            row_y[x] = (((coefs.yr * r + coefs.yg * g + coefs.yb * b) * 1023.0).clamp(0.0, 1023.0) as u16) << 6;
                            row_u[x] = (((coefs.ur * r + coefs.ug * g + coefs.ub * b) * 1023.0 + 512.0).clamp(0.0, 1023.0) as u16) << 6;
                            row_v[x] = (((coefs.vr * r + coefs.vg * g + coefs.vb * b) * 1023.0 + 512.0).clamp(0.0, 1023.0) as u16) << 6;
                        }
                    }
                });
            YuvData {
...
fn encode_av1_frame_cpu(width: usize, height: usize, data: &YuvData, cq: i32, preset: i32, depth: PyColorDepth, chroma: PyChroma) -> PyResult<Vec<u8>> {
    ...
    enc.chroma_sampling = match chroma {
    ...
    let mut ctx: rav1e::Context<u16> = rav1e::Config::new()
        ...
        .new_context()
        .map_err(|e: rav1e::InvalidConfig| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;

    let mut frame = ctx.new_frame();

    if depth == PyColorDepth::EightBit {
        frame.planes[0].copy_from_raw_u8(&data.y, width, 1);
        if chroma == PyChroma::YUV444 {
        ...
        for i in 0..y16.len() { y_unpad[i] = y16[i] >> 6; }
        ...
        if chroma == PyChroma::YUV444 {
            ...
            for i in 0..u16_in.len() { u_unpad[i] = u16_in[i] >> 6; v_unpad[i] = v16_in[i] >> 6; }
    ...
    ctx.send_frame(frame).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(format!("rav1e err: {}", e)))?;
    ctx.flush();
    let mut out = Vec::new();
...
fn encode_av1_frame_gpu(width: usize, height: usize, data: &YuvData, cq: i32, preset: i32) -> PyResult<Vec<u8>> {
    let codec = ffmpeg::encoder::find_by_name("av1_nvenc").ok_or_else(|| {
        pyo3::exceptions::PyRuntimeError::new_err("NVIDIA AV1 Encoder not found")
    })?;
    ...
    unsafe { frame.alloc(data.pixel_format, width as u32, height as u32); }
    ...
    for i in 0..planes {
        ...
        let bytes_per_px = if data.pixel_format == Pixel::P010LE || data.pixel_format == Pixel::YUV444P16LE { 2 } else { 1 };
        ...
        for y in 0..plane_height {
            dst[y*stride..y*stride+row_size].copy_from_slice(&sources[i][y*row_size..y*row_size+row_size]);
        }
    }

    encoder.send_frame(&frame).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(format!("Failed to send frame: {}", e)))?;
    encoder.send_eof().map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(format!("Failed to send EOF: {}", e)))?;

    let mut packet = Packet::empty();
    let mut out = Vec::new();
    while encoder.receive_packet(&mut packet).is_ok() {
        if let Some(d) = packet.data() { out.extend_from_slice(d); }
    }

    if out.is_empty() {
        return Err(pyo3::exceptions::PyRuntimeError::new_err("NVENC returned empty packet"));
    }
...
fn calculate_ssim(
    y_orig: &[u8], orig_stride: usize,
    y_dec: &[u8], dec_stride: usize,
    ...
    let total_ssim: f64 = (0..blocks_y).into_par_iter().map(|by| {
...
fn prepare_trial_frame_8bit(yuv_y: &[u8], width: usize, height: usize, depth: PyColorDepth) -> Vec<u8> {
    let mut y8 = vec![0u8; width * height];
    if depth == PyColorDepth::TenBit {
        let y16: &[u16] = bytemuck::cast_slice(yuv_y);
        for i in 0..y16.len() { y8[i] = (y16[i] >> 8) as u8; }
    } else {
    ...
    if width >= patch_size && height >= patch_size {
        ...
        for &(_, px, py) in &variances {
            if selected.len() >= num_patches { break; }
            ...
            if !overlap { selected.push((px, py)); }
        }
    }
    // Failsafe for small images
    while selected.len() < num_patches { selected.push((0, 0)); }
...
fn check_cq_ssim(cq: i32, trial_y: &[u8], trial_dim: usize, preset: i32, use_gpu: bool) -> PyResult<f64> {
    let mut av1_data = Vec::new();

    if use_gpu {
        let encoder_codec = ffmpeg::encoder::find_by_name("av1_nvenc").unwrap();
        let mut video_ctx = ffmpeg::codec::context::Context::new_with_codec(encoder_codec).encoder().video().unwrap();
        video_ctx.set_width(trial_dim as u32);
        video_ctx.set_height(trial_dim as u32);
        video_ctx.set_format(Pixel::NV12);
        video_ctx.set_time_base((1, 30));
        video_ctx.set_gop(0);
        video_ctx.set_max_b_frames(0);
        let mut options = Dictionary::new();
        ...
        let mut encoder = video_ctx.open_as_with(encoder_codec, options).unwrap();
        let mut frame = ffmpeg::frame::video::Video::empty();
        unsafe { frame.alloc(Pixel::NV12, trial_dim as u32, trial_dim as u32); }
        let stride_y = frame.stride(0);
        let dst_y = frame.data_mut(0);
        for y in 0..trial_dim { dst_y[y*stride_y..y*stride_y+trial_dim].copy_from_slice(&trial_y[y*trial_dim..(y+1)*trial_dim]); }
        let stride_u = frame.stride(1);
        let dst_u = frame.data_mut(1);
        for y in 0..(trial_dim/2) { dst_u[y*stride_u..y*stride_u+trial_dim].fill(128); }

        encoder.send_frame(&frame).unwrap();
        encoder.send_eof().unwrap();
        let mut packet = ffmpeg::codec::packet::Packet::empty();
        while encoder.receive_packet(&mut packet).is_ok() {
            if let Some(d) = packet.data() { av1_data.extend_from_slice(d); }
        }
    } else {
        // CPU cost in fractions of milliseconds (max speed = 10)
        let data = YuvData { y: trial_y.to_vec(), u: vec![128; (trial_dim/2)*(trial_dim/2)*2], v: vec![], pixel_format: Pixel::NV12 };
        av1_data = encode_av1_frame_cpu(trial_dim, trial_dim, &data, cq, 1, PyColorDepth::EightBit, PyChroma::YUV420).unwrap();
    }

    // Decoding (dav1d)
    let decoder_codec = ffmpeg::decoder::find_by_name("libdav1d").unwrap();
    let mut decoder_ctx = ffmpeg::codec::context::Context::new_with_codec(decoder_codec).decoder().video().unwrap();
    ...
    if decoder_ctx.send_packet(&pkt).is_ok() {
        ...
        while decoder_ctx.receive_frame(&mut decoded).is_ok() {
            ...
            computed_ssim = calculate_ssim(trial_y, trial_dim, dec_y, dec_stride, trial_dim, trial_dim);
        }
    }
...
fn estimate_cq(y_ref: &[u8], width: usize, height: usize, depth: PyColorDepth, target_ssim: f64, preset: i32, use_gpu: bool) -> PyResult<i32> {
    ...
    eprintln!("[nvavif_py] Estimate Pass 1 (CQ {}): SSIM {:.4}", cq1, ssim1);
    eprintln!("[nvavif_py] Estimate Pass 2 (CQ {}): SSIM {:.4}", cq2, ssim2);
    ...
    if target_ssim > ssim1 && cq2 < cq1 {
        ...
        if ssim_gain < 0.015 {
            eprintln!("[nvavif_py] SAFEGUARD TRIGGERED: Source contains heavy noise/artifacts. Capping max bitrate.");
    ...
    eprintln!("[nvavif_py] Math Target: {:.2} -> Choosed Clamped CQ: {}", target_cq, final_cq);
...
fn build_yuv<R: PixelReader>(
    ...
    let alpha = if channels == 4 {
        Some(match chroma {
            PyChroma::YUV420 => extract_yuv420(reader, width, height, channels, depth, matrix, true),
            PyChroma::YUV444 => extract_yuv444(reader, width, height, channels, depth, matrix, true),
        })
    } else {
        None
    };
...
fn encode_avif(
    ...
    let use_gpu = match device {
        "gpu" => {
            if !has_gpu { return Err(pyo3::exceptions::PyValueError::new_err("GPU encoding requested but NVENC AV1 not supported")); }
            true
        },
        "cpu" => false,
    ...
    let channels = match input_dtype {
        ...
        _ => return Err(pyo3::exceptions::PyValueError::new_err("Unknown input_dtype")),
    };
    ...
    let (color_yuv, alpha_yuv) = match input_dtype {
        "u8" => build_yuv(&ReaderU8 { data: pixels, channels }, width, height, channels, depth, matrix, chroma),
        "u16" => build_yuv(&ReaderU16 { data: bytemuck::cast_slice(pixels), channels }, width, height, channels, depth, matrix, chroma),
        "f32" => build_yuv(&ReaderF32 { data: bytemuck::cast_slice(pixels), channels }, width, height, channels, depth, matrix, chroma),
        _ => unreachable!(),
    };
    ...
    // Modify CICP metadata to add BT2020 labeling
        ...
        let final_cq = if auto_cq {
            ...
            estimate_cq(&color_yuv.y, width, height, depth, target_ssim, preset, has_gpu).unwrap_or(cq)
        } else {
            cq
        };
        ...
        let color_av1 = if use_gpu {
            ...
            encode_av1_frame_cpu(width, height, &color_yuv, final_cq, preset, depth, chroma)?
        };

        let alpha_av1 = match alpha_yuv {
            Some(d) => {
                let a_cq = alpha_cq.unwrap_or_else(|| (final_cq - 4).clamp(0, 51));
                Some(if use_gpu {
                    encode_av1_frame_gpu(width, height, &d, a_cq, preset)?
                } else {
                    encode_av1_frame_cpu(width, height, &d, a_cq, preset, depth, chroma)?
                })
            },
            None => None
        };
...
fn yuv_to_rgb_parallel(decoded: &ffmpeg::frame::Video) -> PyResult<(Vec<u8>, usize, usize, usize)> {
    ...
    let (has_alpha, is_10bit) = match format {
        ...
        other => return Err(pyo3::exceptions::PyRuntimeError::new_err(format!("Unsupported: {:?}", other))),
    };

    let is_444 = matches!(format, Pixel::YUV444P | Pixel::YUV444P10LE | Pixel::YUVA444P | Pixel::YUVA444P10LE);
    let is_422 = matches!(format, Pixel::YUV422P | Pixel::YUV422P10LE | Pixel::YUVA422P);
    ...
    let a_data = if has_alpha { Some(decoded.data(3)) } else { None };
    ...
    let (c_cr_r, c_cb_g, c_cr_g, c_cb_b) = match color_space {
        ...
        ffmpeg::color::Space::SMPTE170M | ffmpeg::color::Space::BT470BG => {
            (1436, 352, 731, 1815)
        },
    ...
    output.par_chunks_mut(w * channels).enumerate().for_each(|(y_row, row_out)| {
        for x in 0..w {
            let (cx, cy) = if is_444 { (x, y_row) }
                           else if is_422 { (x / 2, y_row) }
                           else { (x / 2, y_row / 2) };

            let (y_val, cb_val, cr_val) = if is_10bit {
                let y16 = u16::from_le_bytes([y_data[y_row * y_stride + x * 2], y_data[y_row * y_stride + x * 2 + 1]]) as i32;
                let u16v = u16::from_le_bytes([u_data[cy * u_stride + cx * 2], u_data[cy * u_stride + cx * 2 + 1]]) as i32;
                let v16v = u16::from_le_bytes([v_data[cy * v_stride + cx * 2], v_data[cy * v_stride + cx * 2 + 1]]) as i32;
...
fn decode_avif(py: Python<'_>, path: &str, threads: usize) -> PyResult<Py<PyAny>> {
    ...
    let (data, width, height, channels) = py.detach(move || -> PyResult<(Vec<u8>, usize, usize, usize)> {
        let mut options = ffmpeg::Dictionary::new();
        options.set("probesize", "4096");
        options.set("analyzeduration", "0");

        let mut ictx = ffmpeg::format::input_with_dictionary(&path_owned, options).map_err(|e| {
            pyo3::exceptions::PyIOError::new_err(format!("Failed to open AVIF file {}: {}", path_owned, e))
        })?;
        ...
        decoder_builder.set_threading(ffmpeg::threading::Config {
            ...
            count: threads, // 0 = Auto: For the ML DataLoader, it's recommended to set 1 or 2.
        });

        let mut decoder = decoder_builder.open_as(codec)
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(format!("Decoder open failed: {}", e)))?
            .video()
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(format!("Video context failed: {}", e)))?;

        let mut decoded = ffmpeg::frame::Video::empty();

        for (st, packet) in ictx.packets() {
            if st.index() == stream_idx {
                if decoder.send_packet(&packet).is_ok()
                    && decoder.receive_frame(&mut decoded).is_ok()
                {
                    return yuv_to_rgb_parallel(&decoded);
                }
            }
        }

        if decoder.send_eof().is_ok() && decoder.receive_frame(&mut decoded).is_ok() {
            return yuv_to_rgb_parallel(&decoded);
        }

        Err(pyo3::exceptions::PyRuntimeError::new_err("Could not decode any frame from AVIF"))
    })?;
