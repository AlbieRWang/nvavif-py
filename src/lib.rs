...
fn encode_av1_frame_cpu(
    ...
    // NOTE: do not enable the rav1e "asm" feature (Cargo.toml). Its
    // nasm-built SIMD kernels silently corrupt reconstruction pixels on this
    // toolchain for hard-edged monochrome content — decoded alpha came back
    // as blocky ramps / all-zero planes, and rav1e's cdef.rs:95
    // `p >> coeff_shift <= 255` debug_assert fires on the poisoned pixels in
    // debug builds while release builds (asserts compiled out) emit the
    // corrupt bitstream silently. Root cause + verification:
    // DEVELOPMENT_NOTES 11.4, uvtest/diag_alpha_bug.py, and the
    // alpha_dbg_tests regression below.
...
mod alpha_dbg_tests {
    ...
    /// Reproduces the alpha corruption offline: dump the extracted alpha
    /// plane and the rav1e bitstream for Cs400 vs Cs420 so the failing
    /// stage can be identified without a Python roundtrip.
    fn run_case(name: &str, w: usize, h: usize) {
        let dir = concat!(env!("CARGO_MANIFEST_DIR"), "/uvtest/out/alpha_dbg");
        let raw = std::fs::read(format!("{dir}/{name}.raw")).unwrap();
        let reader = ReaderU8 { data: &raw, channels: 4 };
        let alpha = extract_yuv420(&reader, w, h, 4, PyColorDepth::EightBit, PyColorMatrix::Bt709, true);
        std::fs::write(format!("{dir}/{name}.plane"), &alpha.y).unwrap();

        let obu400 = encode_av1_frame_cpu(w, h, &alpha, 16, 2, PyColorDepth::EightBit, PyChroma::YUV420, true).unwrap();
        std::fs::write(format!("{dir}/{name}.cs400.obu"), &obu400).unwrap();

        let mut color = YuvData { y: alpha.y.clone(), u: vec![128u8; w * h], v: vec![128u8; w * h], pixel_format: Pixel::YUV444P };
        let _ = &mut color;
        let obu420 = encode_av1_frame_cpu(w, h, &color, 16, 2, PyColorDepth::EightBit, PyChroma::YUV444, false).unwrap();
        std::fs::write(format!("{dir}/{name}.cs420.obu"), &obu420).unwrap();
        println!("{name}: plane={} cs400={} cs420={} bytes", alpha.y.len(), obu400.len(), obu420.len());
    }

    #[test]
    fn dump_alpha_stage() {
        let dir = concat!(env!("CARGO_MANIFEST_DIR"), "/uvtest/out/alpha_dbg");
        for (name, w, h) in [("half", 256usize, 256usize), ("mask1", 500, 500), ("control", 500, 500)] {
            let mask_path = std::path::Path::new(&dir).join(format!("{name}.mask"));
            if !mask_path.exists() {
                // Input fixtures are generated by uvtest/diag_alpha_bug.py
                // setup; without them this is a no-op rather than a failure.
                println!("fixtures missing, skipping {name}");
                continue;
            }
            run_case(name, w, h);
            let plane = std::fs::read(format!("{dir}/{name}.plane")).unwrap();
